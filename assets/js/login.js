window.addEventListener('elementor/frontend/init', function () {
    ...
    elementorFrontend.hooks.addAction('frontend/element_ready/efbLoginFormElementor.default', function ($scope) {
        elementorEditorCustom = document.querySelector('.elementor-editor-active')
        
        if(elementorEditorCustom){
            changeInputsByClass()
        }
...
window.addEventListener('emu_form_ready', () => {
    ...
    function verifyPassword(password, confirm) {
        ...
        if (!passwordRegex.test(password)) {
            efbReturnResponse('Para sua segurança, a senha deve ter pelo menos 8, a 16 caracteres. Letras, números e símbolos (@#$%^&+=!).', 'emu-notices-danger');
            return false;
        }
